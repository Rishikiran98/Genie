            ...
            for node in frontier:
                ...
                for tactic in tactics:
                    ...
                    if result.proof_complete:
                        return SearchResult(status="solved", node=next_node, best_partial=node.history)

            if not candidates:
                return SearchResult(status="exhausted", node=None, best_partial=[])
            candidates.sort(key=lambda n: n.score, reverse=True)
            frontier = candidates[: self.config.beam_width]

        best = frontier[0] if frontier else None
        return SearchResult(status="exhausted", node=best, best_partial=best.history if best else [])
